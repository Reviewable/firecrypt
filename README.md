...
* `keySignature`: a key signature generated by a previous call to `initializeEncryption`, to verify if the key matches without revealing it.
...
If the `algorithm` is `aes-siv` and you don't pass a `keySignature`, then `initializeEncryption` will return a key signature that can be used to verify whether another key matches.  If you do pass in `keySignature` then the function will return `true` the `key` matches the signature and `false` otherwise.  This can be useful if the key is distributed as part of a session, and you want to check if you need to invalidate the session because the key has been rotated.
